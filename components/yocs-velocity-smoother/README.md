...
# Example usage
```
docker run -it --network=host cognimbus/yocs_velocity_smoother:melodic roslaunch yocs_velocity_smoother velocity_smoother_with_params.launch accel_lim_v:=0.3 accel_lim_w:=3.5 decel_factor:=1 frequency:=20 speed_lim_v:=0.8 speed_lim_w:=5.4 robot_feedback:=0
```

...
#include <lynx_navigation/trajectory/simulator/models/IMotionModel.h>
...
public:
    ...
    bool operator==(const AckermannModel& other) {
        return fabs(this->getSpeed() - other.getSpeed()) < 0.001 && 
                fabs(this->getWheelSeparation() - other.getWheelSeparation()) < 0.001 && 
                fabs(this->getSteeringAngle() - other.getSteeringAngle()) < 0.001;
    }
	...
	inline ackermann_msgs::AckermannDriveStamped getAckermannMessage() const {
		...
		message.drive.steering_angle = steeringAngle_ * 1.0;
		return message;
	}

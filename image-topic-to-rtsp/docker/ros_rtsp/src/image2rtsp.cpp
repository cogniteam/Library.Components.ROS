...
void Image2RTSPNodelet::onInit() {
    ...
    nh.getParam("numOfStreams", this->numOfStreams_);
    nh.getParam("mountpoint", this->mountpoint_name_);
    nh.getParam("port", this->port_);
    nh.getParam("topicPrefix", this->topic_prefix_);
    ...
    rtsp_server = rtsp_server_create(port_);

    for (int i=1; i<=numOfStreams_; i++) {
        mountpoint = mountpoint_name_ + std::to_string(i);
        bitrate = "500";
        ...
        NODELET_INFO("Stream available at rtsp://%s:%s%s", gst_rtsp_server_get_address(rtsp_server), port_.c_str(), mountpoint.c_str());
    }
...
void Image2RTSPNodelet::url_connected(string url) {
    ...
    for (int i=1; i<= this->numOfStreams_; i++) {
        type = "topic";
        mountpoint = this->mountpoint_name_ + std::to_string(i);
        source = topic_prefix_   + std::to_string(i);
        if (url==mountpoint) {
